...
- **TUI**: list-first under [examples/tui/](examples/tui/). Screen names are an enum at [examples/tui/screens/screen.js](examples/tui/screens/screen.js); add to that enum rather than introducing string literals. Input handling uses a declarative keymap layer: key descriptors in [examples/tui/keys.js](examples/tui/keys.js) (`Key.*` + `char(c)`), semantic verbs in [examples/tui/actions.js](examples/tui/actions.js), reusable cross-screen key sets (`BACK_KEYS`, `LIST_UP_KEYS`, `LIST_DOWN_KEYS`, `LIKE_KEYS`, `DISLIKE_KEYS`) in [examples/tui/bindings.js](examples/tui/bindings.js), and a generic `useKeymap(bindings, handlers)` hook in [examples/tui/useKeymap.js](examples/tui/useKeymap.js). Screens declare a `[{ keys, action, when? }]` table instead of branching on raw `useInput` arguments — adding a new key/action is one row of data, not another `else if`. Reuse the shared key-set constants from `bindings.js` for navigation that should stay consistent (back, list cursor, like/dislike); only the action and `when` clause stay at the call site. New verbs go in `actions.js`; never inline string literals or raw `key.escape` checks.
