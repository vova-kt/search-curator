...
from events_curator.models import Feedback, SavedQuery, User
...
def _authenticate():
    async def _action(storage, _pipeline):
...
def _new_query_form(principal) -> None:
...
def _run_query(query, principal) -> None:
    try:
...
def _send_feedback(query, principal, result, kind: FeedbackKind) -> None:
    feedback = Feedback(
...
def _result_row(query, principal, result) -> None:
...
def _feedback_section(query, principal) -> None:
